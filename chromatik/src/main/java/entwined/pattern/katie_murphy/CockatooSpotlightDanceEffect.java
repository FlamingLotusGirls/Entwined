...
@LXCategory("Custom")
public class CockatooSpotlightDanceEffect extends LXEffect {
    ...
    final BooleanParameter CockatooSpotlightDanceParam =
    ...
    // Cached points for CockatooSegment + Spotlight + NestSurface
    private final List<LXPoint> taggedPoints = new ArrayList<>();

    public CockatooSpotlightDanceEffect(LX lx) {
        super(lx);
        addParameter("CockatooSpotlightDance", CockatooSpotlightDanceParam);
        addParameter("palette", paletteParam);

        CockatooSpotlightDanceParam.addListener(p -> {
            if (CockatooSpotlightDanceParam.getValueb() && !danceActive) {
                danceActive = true;

...
@LXCategory("Custom")
public class MagpieSpotlightDanceEffect extends LXEffect {
    ...
    final BooleanParameter MagpieSpotlightDanceParam =
    ...
    // Cached points for MagpieSegment + Spotlight + NestSurface
    private final List<LXPoint> taggedPoints = new ArrayList<>();

    public MagpieSpotlightDanceEffect(LX lx) {
        super(lx);
        addParameter("MagpieSpotlightDance", MagpieSpotlightDanceParam);
        addParameter("palette", paletteParam);

        MagpieSpotlightDanceParam.addListener(p -> {
            if (MagpieSpotlightDanceParam.getValueb() && !danceActive) {
                danceActive = true;

...
public abstract class TSTriggerablePattern extends LXPattern
  implements Triggerable {

  public static final int PATTERN_MODE_PATTERN = 0; // not implemented
  public static final int PATTERN_MODE_TRIGGER = 1; // calls the run loop only
                                                    // when triggered
  public static final int PATTERN_MODE_FIRED = 2;   // like triggered, but
                                                    // pattern must disable
  ...
  // Note here that I'm using onParameter change to actually do the processing
  // of a trigger
  // rather than onTrigger/onRelease. This layer of indirection - through an
  // otherwise
  // superfluous BooleanParameter - is necessary because record and playback
  // only captures
  // events that occur on Parameters. So during playback I *wont* get the
  // onTrigger event
  // from the APC40 that in turn, triggers the onParameterChanged callback; on
  // the other hand,

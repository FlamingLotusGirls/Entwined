...
public class Entwined implements LXStudio.Plugin {
  ...
  private void registerEntwinedContent(LX lx) {
    // NB - all available patterns - including those available on the iPad -
    ...
    lx.registry
      .addPattern(entwined.pattern.adam_n_katie.ExpandingCircles.class);
    ...
    lx.registry
      .addPattern(entwined.pattern.adam_n_katie.VerticalColorWaves.class);
    ...
    lx.registry
      .addPattern(entwined.pattern.clover_watson.SpinningStained.class);
    ...
    lx.registry
      .addPattern(entwined.pattern.eric_gauderman.DiscreteColors.class);
    lx.registry.addPattern(entwined.pattern.eric_gauderman.FreeFall.class);
    lx.registry.addPattern(entwined.pattern.eric_gauderman.HavenSolids.class);
    ...
    // NB - I note that the following are containers for LXEffects, not
    // LXEffects themselves.
    // We communicate with them to enable and disable their per-component child
    // effects
    ...
    // XXX - I'm guessing that these - the children of the above - don't have to
    // be registered either, since they are never
    ...
    lx.registry
      .addEffect(entwined.pattern.interactive.InteractiveHSVEffect.class);
    ...
    lx.registry
      .addEffect(entwined.pattern.kyle_fleming.AcidTripTextureEffect.class);
    lx.registry.addPattern(entwined.pattern.kyle_fleming.Brightness.class);
    lx.registry
      .addEffect(entwined.pattern.kyle_fleming.BrightnessScaleEffect.class);
    lx.registry.addPattern(entwined.pattern.kyle_fleming.CandyCloud.class);
    lx.registry
      .addEffect(entwined.pattern.kyle_fleming.CandyCloudTextureEffect.class);
    ...
    lx.registry
      .addEffect(entwined.pattern.kyle_fleming.ColorStrobeTextureEffect.class);
    ...
    lx.registry
      .addEffect(entwined.pattern.kyle_fleming.FadeTextureEffect.class);
    ...
    lx.registry
      .addPattern(entwined.pattern.kyle_fleming.RandomColorGlitch.class);
    ...
    lx.registry
      .addEffect(entwined.pattern.kyle_fleming.TurnOffDeadPixelsEffect.class);
    ...
    lx.registry
      .addPattern(entwined.pattern.mattaniah.OscillatingDarkRing.class);
  ...
  private void setupMasterEffects() {
    ...
    masterBrightnessEffect = setupMasterEffectWithName(lx,
      BrightnessScaleEffect.class, masterBrightnessName);
    autoplayBrightnessEffect = setupMasterEffectWithName(lx,
      BrightnessScaleEffect.class, autoplayBrightnessName);
  }

  private void configureTriggeredEffects() {
    setupMasterEffects();     // sets up standard effects on master channel
    configureTriggerables();  // Configures triggerable effects, including
                              // effects for APC40
  }
  ...
  public void initialize(LX lx) {
    ...
    // XXX - it *was* also used to do autoplayback and (I think) recording,
    // which is
    // a crucial feature.
        ...
        private void noteReceived(MidiNote note, boolean on) {
          ...
          if (triggerables != null) {
            ...
            if (pitch >= APC40.CLIP_LAUNCH && pitch <= APC40.CLIP_LAUNCH_MAX) {
              triggerables.grid[pitch - APC40.CLIP_LAUNCH][channel]
                .setValue(on);
            } else if (pitch == APC40.CLIP_STOP) {
              triggerables.grid[Triggerables.NUM_COLS - 1][channel]
                .setValue(on);
            } else if (pitch >= APC40.SCENE_LAUNCH
              && pitch <= APC40.SCENE_LAUNCH_MAX) {
              triggerables.grid[pitch
                - APC40.SCENE_LAUNCH][Triggerables.NUM_ROWS - 1].setValue(on);
            }
          }
        }
      });
    ...
    lx.addProjectListener(new LX.ProjectListener() {
      ...
      public void projectChanged(File file, Change change) {
        if (change == Change.NEW || change == Change.OPEN) {
          ...
          engineController = new EngineController(lx, entwined);  // XXX might
                                                                  // want to
                                                                  // have a
                                                                  // listener on
                                                                  // the
          ...
          // Set up high level iPad Server. Uses the iPadController to actually
          // do the work.
          ...
          if (recordings != null) {
            // TODO - perhaps come up with a more elegant solution here for
            // specifying
  ...
  void configureTriggerables() {
    ...
    // Standard patterns
    // One-shot patterns
    // Patterns that use a parameter in addition to the fader for fading in and
    // out

    // other breadcrumbs:
    // the "setAction" will register an action for the grid of APC40 / Triggers
    // array
    // 0,0 is the upper left corner <row>,<column>; so 0,1 is the second button
    // on the top row
    // 1,0 is the first button on the second row

    // how to add one: look at the textures on the master channel.
    // find one you want. Look up its class and find a parameter you want to
    // change.
    // Follow the pattern.

    // Instead of turning the action on or off, let's change the color.
    // ReleaseDisables indicates that this is an effect that is always going to
    // be on - we're just playing with its values.
    CandyCloudTextureEffect cctEffect = Entwined.findMasterEffect(lx,
      CandyCloudTextureEffect.class);
    triggerables.setAction(0, 0,
      new EventTrigger(cctEffect, cctEffect.amount, 0.7)
        .releaseDisables(false));
    cctEffect.enabled.setValue(false);

    // straight up on/off on the effect - turn on or off the standard color
    // effect
    // you don't want to do this. Effects have to be on all the time.
    // triggerables.setAction(0,0, new
    // EventTrigger(Entwined.findMasterEffect(lx, ColorEffect.class)));
    ...
    triggerables.setAction(0, 1,
      new EventTrigger(colorEffect, colorEffect.hueShift, 1.0)
        .releaseDisables(false));
    colorEffect.enabled.setValue(false);

    // The same sort of thing can be used for triggering patterns on the
    // 'Events' channel..
    ...
    triggerables.setAction(0, 2,
      new EventTrigger(bubbles, bubbles.ballCount, 50.0));  // Lots of bubbles
    triggerables.setAction(0, 3,
      new EventTrigger(bubbles, bubbles.ballCount, 5.0));   // Not so many
                                                            // bubbles
    bubbles.enabled.setValue(false);

    // Let's set up a lot of Wisps as well. That seemed to be popular in the
    // previous version of LX
    // Here I construct a large number of slightly different versions.
    Wisps downward_yellow_wisp = Entwined.findPatternWithName(effectsChannel,
      Wisps.class, "Wisp: yellow");
    ...
    Wisps colorful_wisp_storm = Entwined.findPatternWithName(effectsChannel,
      Wisps.class, "Wisp: storm");
    ...
    Wisps multidirection_wisps = Entwined.findPatternWithName(effectsChannel,
      Wisps.class, "Wisp: multi");
    ...
    Wisps rainstorm_wisps = Entwined.findPatternWithName(effectsChannel,
      Wisps.class, "Wisp: rain");
    ...
    Wisps twister_wisps = Entwined.findPatternWithName(effectsChannel,
      Wisps.class, "Wisp: twister");
    ...
    // Here's another way of setting things up if the pattern isn't designed for
    // triggers Very simple!
    triggerables.createPatternAction(1, 0,
      Entwined.setupTriggerablePattern(lx, effectsChannel, Cells.class));
    ...
    ScrambleEffect scrambleEffect = Entwined.findMasterEffect(lx,
      ScrambleEffect.class);
    triggerables.setAction(1, 7,
      new EventTrigger(scrambleEffect, scrambleEffect.amount, 1.0)
        .releaseDisables(false));
    scrambleEffect.enabled.setValue(false);

    // 1.0 amount is a bit harsh
    ColorStrobeTextureEffect cstEffect = Entwined.findMasterEffect(lx,
      ColorStrobeTextureEffect.class);
    triggerables.setAction(1, 8,
      new EventTrigger(cstEffect, cstEffect.amount, 0.7)
        .releaseDisables(false));
  ...
  class EventTrigger implements Triggerable {
    ...
    EventTrigger(LXDeviceComponent effect,
      LXListenableNormalizedParameter amountControl, double onAmount) {
      this.effect = effect;
   ...
   * configureCanopy
   *
   * Canopy is the system used to control per-component interactivity from
   * external devices, such as the web interface on a phone, or an rPi hooked up
   * to some sensor.
   *
   * The configuration here creates these special interactive effects and sets
   * up the Canopy controller.
   *
   * Note that currently the pattern for effects used by Canopy is a little
   * non-obvious. For each type of effect we want to have available to Canopy,
   * we create a separate global effect for each component. (So N types of
   * effects, M components, N*M global effects). This probably needs a little
   * more thinking through - there are probably ways of abstracting it so we can
   * use a config file, have a CanopyEffect class, and don't have to create
   * quite so many global effects.
   */

  void configureCanopy() {
    System.out.println("Configuring canopy, lx is " + lx);

    interactiveCandyChaosEffect = setupMasterEffect(lx,
      InteractiveCandyChaosEffect.class);
    interactiveDesaturationEffect = setupMasterEffect(lx,
      InteractiveDesaturationEffect.class);
    interactiveRainbowEffect = setupMasterEffect(lx,
      InteractiveRainbowEffect.class);
    ...
    // tell the canopyController what it should be up to. The system alternates
    // between running
    ...
    // This on/off thing was a requirement of the Parks Department in times of
    // Covid, to
    // keep people from enjoying the sculpture too much. If pausePauseMinutes is
    // 0, it
    // doesn't pause.
   ...
   * pattern.setTransition(t); pattern.readableName = name;
   * patterns.add(pattern); } // XXX - do I want this to come back? This is an
   * interesting little bit of code.
   */
   ...
   * the iPad app. These are the 'base channels' and the 'server channels,
   * respectively.
   *
   * In addition, there is a single channel for triggerable effects, the
   * effectChannel.
   *
   * This code is called at startup to make sure that we have enough channels
   * allocated. It will also populate any new server channels with the default
   * iPad patterns.
   */
  ...
  void configureChannels() {
    ...
    log("Configure channels - have " + numCurrentChannels
      + " channels, base channels is " + Config.NUM_BASE_CHANNELS
      + ", server channels is " + Config.NUM_SERVER_CHANNELS);
    ...
    numAddedChannels = 0;
    if (numCurrentChannels < Config.NUM_SERVER_CHANNELS
      + Config.NUM_BASE_CHANNELS) {
      for (int i = 0; i < Config.NUM_SERVER_CHANNELS + Config.NUM_BASE_CHANNELS
        - numCurrentChannels; i++) {
        LXChannel channel = lx.engine.mixer.addChannel();
        channel.setPatterns(getIPadPatterns(lx));  // NB - This is
                                                   // intentionally. Only
                                                   // configure if not
    ...
    for (LXAbstractChannel abstractChannel : lx.engine.mixer.channels) {
      if (abstractChannel instanceof LXChannel) {
        if ((currentChannelIdx >= Config.NUM_BASE_CHANNELS)
          && (currentChannelIdx < Config.NUM_BASE_CHANNELS
            + Config.NUM_SERVER_CHANNELS)) {
          abstractChannel.label.setValue("IPad");
          abstractChannel.label
            .setDescription("Patterns and channels used by iPad application");
          abstractChannel.fader.setValue(1);
        } else if (currentChannelIdx == Config.NUM_BASE_CHANNELS
          + Config.NUM_SERVER_CHANNELS) {
          abstractChannel.label.setValue("Effects");
          abstractChannel.label.setDescription(
            "Channel handling patterns that produce triggerable effects. Set up by plugin, do not modify");
   ...
   * getIPadPatterns
   *
   * Construct an array consisting of the default iPad patterns, as specified in
   * the config file. This array is used to fill out the patterns available on
   * the Server channels, which are used by the iPad app.
   */
  ...
  LXPattern[] getIPadPatterns(LX lx) {
    ...
    for (Class<?> patternClass : Config.iPadPatterns) {
      try {
        iPadPatterns
          .add(lx.instantiatePattern(patternClass.asSubclass(LXPattern.class)));
  ...
  // XXX - Can I get a channel listener for these channels, and a global
  // listener, so I can update
  ...
  public void onUIReady(LXStudio lx, UI ui) {
    // NOTE(mcslee): potentially something here if we ever want custom UI
    // components, but
  ...
  public static <T extends LXEffect> T setupMasterEffect(LX lx,
    Class<T> clazz) {
    ...
    if (effect == null) {
      try {
        ...
        System.out
          .println("Constructor for class " + clazz + "failing, continuing on");
      }
    }
    return (T) effect;
  }

  // Utility function for checking for the existence of an effect with this
  // particular *name*, as well
  // as class.
  //
  @SuppressWarnings("unchecked")
  public static <T extends LXEffect> T setupMasterEffectWithName(LX lx,
  ...
  @SuppressWarnings("unchecked")
  public static <T extends LXPattern> T setupTriggerablePattern(LX lx,
    LXChannel channel, Class<T> clazz) {
    ...
    if (pattern == null) {
      try {
        ...
        e.printStackTrace();
        return null;
      } // XXX would like to be able to specify constructors in many instances.
        // Erk
    }
  ...
  @SuppressWarnings("unchecked")
  public static <T extends LXEffect> T findMasterEffectWithName(LX lx,
    Class<T> clazz, String name) {
  ...
  @SuppressWarnings("unchecked")
  public static <T extends LXEffect> T findEffectWithName(LXBus bus,
    Class<T> clazz, String name) {
    for (LXEffect effect : bus.effects) {
      if (effect.getClass().equals(clazz)
        && effect.label.getString().equals(name)) {
        return (T) effect;
      }
    }
    return null;
  ...
  @SuppressWarnings("unchecked")
  public static <T extends LXPattern> T findPattern(LXChannel channel,
    Class<T> clazz) {
  ...
  @SuppressWarnings("unchecked")
  public static <T extends LXPattern> T findPatternWithNameOnly(
    LXChannel channel, String name) {
    for (LXPattern pattern : channel.patterns) {
  ...
  @SuppressWarnings("unchecked")
  public static <T extends LXPattern> T findPatternWithName(LXChannel channel,
    Class<T> clazz, String name) {
    for (LXPattern pattern : channel.patterns) {
      if (pattern.getClass().equals(clazz)
        && pattern.label.getString().equals(name)) {
        return (T) pattern;
      }
    }
    return null;

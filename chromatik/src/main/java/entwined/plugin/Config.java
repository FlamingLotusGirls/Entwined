...
public final class Config {
  ...
  // NB - the following 'pause' fields are vestigial, however, they are used in
  // the
  ...
  // "https://entwined-api.charliestigler.com/lx";
  ...
  /*
   * static { groups.put("shrubies", new String[] {"shrub-1", "shrub-2"}); }
   */

  // These are the patterns available on the ipad, in the order that they are
  ...
  static final Class<?>[] iPadPatterns = {
    ...
    entwined.pattern.anon.SeeSaw.class, entwined.pattern.irene_zhou.Cells.class,
    ...
    entwined.pattern.misko.Circles.class, entwined.pattern.misko.LineScan.class,
  ...
  static void Init(String configFilename) {
    ...
    try (FileReader fr = new FileReader(configFile)) {
      ...
      if (obj.has("interactiveGroups")) {
        JsonObject interactiveGroups = obj.get("interactiveGroups")
          .getAsJsonObject();
        for (String key : interactiveGroups.keySet()) {
          JsonArray componentsJson = interactiveGroups.get(key)
            .getAsJsonArray();
          ...
          for (int j = 0; j < componentsJson.size(); ++j) {
            ...
            System.out.println(
              "Adding component " + components[j] + " to group " + key);
          }

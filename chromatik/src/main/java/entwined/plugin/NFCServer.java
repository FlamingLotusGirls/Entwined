...
public class NFCServer implements LXLoopTask {
  ...
  // Base structures:
  // The system enables a number of NFCPatterns that can be triggered by the NFC
  // cards.
  // NFC patterns are either one-shots, global modifiers, or patterns running on
  // one of
  // the server pattern channels.
  // A OSC style trigger will generate an NFCTrigger object, specifying which
  // NFCPattern
  // being effected, whether it's being turned on or off, and which channel it's
  // being
  // run on.
  // One shot patterns will only be active for a short period of time. They are
  // managed by
  ...
  public enum NFCPatternType {
    BASE_PATTERN, ONE_SHOT, GLOBAL_MODIFIER
  }
  ...
  NFCServer(LX lx, EngineController engineController) {
    ...
    this.ATTRACT_MODE_DELTA_MS = Config.attractModeEnable
      ? Config.attractModeTimeout
      : 0;
    this.engineController = engineController;
    this.lx = lx;
    short port = Config.NFCPort != -1 ? Config.NFCPort : 7777;
    this.numActivities = Config.NFCNumActivities != -1 ? Config.NFCNumActivities
      : 4;
    ...
    System.out.println("NFC Server Init, Attract Mode " + this.attractModeEnable
      + ", timeout " + this.ATTRACT_MODE_DELTA_MS);
  }

  private NFCPattern createOneShot(String name, Class clazz) {
    NFCPattern pattern = new NFCPattern(name, NFCPatternType.ONE_SHOT,
      engineController.setupPatternEffect(clazz));
  ...
  private void setupPatternMap() {
    ...
    this.patternMap.put("TwisterGlobal",
      new NFCPattern("TwisterGlobal", NFCPatternType.BASE_PATTERN));
    this.patternMap.put("MarkLottor",
      new NFCPattern("MarkLottor", NFCPatternType.BASE_PATTERN));
    this.patternMap.put("Ripple",
      new NFCPattern("Ripple", NFCPatternType.BASE_PATTERN));
    this.patternMap.put("Lattice",
      new NFCPattern("Lattice", NFCPatternType.BASE_PATTERN));
    this.patternMap.put("Voronoi",
      new NFCPattern("Voranoi", NFCPatternType.BASE_PATTERN));
    this.patternMap.put("GalaxyCloud",
      new NFCPattern("GalaxyCloud", NFCPatternType.BASE_PATTERN));
    this.patternMap.put("Fire",
      new NFCPattern("Fire", NFCPatternType.BASE_PATTERN));
    this.patternMap.put("AcidTrip",
      new NFCPattern("AcidTrip", NFCPatternType.BASE_PATTERN));
    this.patternMap.put("SparkleHelix",
      new NFCPattern("SparkleHelix", NFCPatternType.BASE_PATTERN));
    this.patternMap.put("Cells",
      new NFCPattern("Cells", NFCPatternType.BASE_PATTERN));
    this.patternMap.put("RingoDown",
      new NFCPattern("RingoDown", NFCPatternType.BASE_PATTERN));
    this.patternMap.put("Explosions",
      new NFCPattern("Explosions", NFCPatternType.BASE_PATTERN));
    this.patternMap.put("ColoredLeaves",
      new NFCPattern("ColoredLeaves", NFCPatternType.BASE_PATTERN));
    this.patternMap.put("Fumes",
      new NFCPattern("Fumes", NFCPatternType.BASE_PATTERN));
    // One shots
    this.patternMap.put("Lightning",
      createOneShot("Lightning", Lightning.class));
    ...
    this.patternMap.put("ColorStrobe",
      createOneShot("ColorStrobe", ColorStrobe.class));
    this.patternMap.put("Rain", createOneShot("Rain", Rain.class));
    // Globals
    this.patternMap.put("Static",
      new NFCPattern("static", NFCPatternType.GLOBAL_MODIFIER));
    this.patternMap.put("Blur",
      new NFCPattern("blur", NFCPatternType.GLOBAL_MODIFIER));
    this.patternMap.put("Speed",
      new NFCPattern("speed", NFCPatternType.GLOBAL_MODIFIER));
    this.patternMap.put("Scramble",
      new NFCPattern("scramble", NFCPatternType.GLOBAL_MODIFIER));

    // XXX - the parameters for the various effects should be set up in the
    // project file...
    // XXX - make absolutely sure (again) that the effects in the effects
    // channel can happen simultaneously
  }
  ...
  protected void handleOneShots() {
    ...
    for (NFCOneShot oneShot : oneShotMap.values()) {
      if (oneShot.enabled && oneShot.timeout < curTime) {
        ...
        System.out.println("Releasing one shot, timeout  " + oneShot.timeout
          + " cur time " + curTime);
  ...
  protected void triggerOneShot(String patternName) {
    ...
    if (oneShot.enabled != true) {
      ...
      oneShot.triggerable.onTriggered();  // XXX better understand what's going
                                          // on here... XX
    }
  ...
  public void loop(double deltaMs) {
    ...
    autoPlaying = false;
    attractModeTimeout = EntwinedUtils.millis() + ATTRACT_MODE_DELTA_MS;
    System.out.println("Attract mode enable is " + this.attractModeEnable
      + ", timeout is " + this.attractModeTimeout + ", currentTime is "
      + EntwinedUtils.millis());
    ...
    if (!trigger.onOff) {  // Turn off
      ...
      if (trigger.pattern.type == NFCPatternType.BASE_PATTERN) {
        ...
        engineController.setChannelPattern(channelIdx,
        ...
        engineController.setChannelVisibility(
          engineController.baseChannelIndex + this.numActivities, 1.0);
  ...
  private void modifyIntensities() {
    ...
    for (NFCPattern pattern : this.activities) {
      if (pattern != null && pattern.type == NFCPatternType.BASE_PATTERN) {
        numBasePatterns++;
      }
    }
  ...
  private NFCTrigger parseNFCCommand(String command) {
    ...
    NFCPattern pattern;
    String patternName;
    // Format of the command is /channel/<channelid>/pattern/<patternName> ,s
    // [T|F]
    try {
      ...
      channelIdx = Character
        .getNumericValue(command.charAt(channelStarterIdx + 9));

      if (!command.substring(channelStarterIdx + 10, channelStarterIdx + 19)
        .equals("/pattern/")) {
        throw new RuntimeException(
          "Does not contain '/pattern/' at correct location");
      }
      ...
      if (pattern == null) {
        throw new RuntimeException(
          "Pattern name " + patternName + " not found");
      }

    } catch (Exception e) {
      System.out.println(
        "Could not parse NFC command " + command + ", " + e.getMessage());
      return null;
    }
